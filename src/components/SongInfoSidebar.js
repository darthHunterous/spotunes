...
import SongInfoEditFormModal from './SongInfoEditFormModal';

export default function SongInfoSidebar({ showInfoSong, setShowInfoSong, songData, setSongData, setAddToPlaylistChosenSong, setShowAddToPlaylistModal }) {
  const [showSongInfoEditFormModal, setShowSongInfoEditFormModal] = useState(false);
  ...
  function handleEditButton(showInfoSong) {
    setShowSongInfoEditFormModal(true);
  }

  function handleDeleteButton(spotifyID) {
    if (window.confirm('Are you sure you want to delete this song?')) {
      const adjustedSongData = songData.filter((song) => song.spotifyID !== spotifyID);
      setSongData(adjustedSongData);
  ...
  return (
    <>
            ...
            <Card>
              <Card.Body className="p-0">
                <ListGroup variant="flush">
                  ...
                  <ListGroup.Item>
                    <Button variant="primary" size="sm" className="w-100" onClick={() => handleEditButton(showInfoSong)}>
                  ...
                  <ListGroup.Item>
                    <Button variant="danger" size="sm" className="w-100" onClick={() => handleDeleteButton(showInfoSong.spotifyID)}>
      ...
      <SongInfoEditFormModal
        showSongInfoEditFormModal={showSongInfoEditFormModal}
        setShowSongInfoEditFormModal={setShowSongInfoEditFormModal}
